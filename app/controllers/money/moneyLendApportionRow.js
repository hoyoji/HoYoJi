...
$.onWindowOpenDo(function() {
	if ($.$model.xGet("moneyLend").xGet("ownerUser") !== Alloy.Models.User) {
...
$.makeContextMenu = function() {
	...
	menuSection.add($.createContextMenuItem("移除成员", function() {
		deleteApportion();
	}, $.$model.xGet("moneyLend").xGet("ownerUser") !== Alloy.Models.User));
	menuSection.add($.createContextMenuItem("设为均摊", function() {
		setToAverage();
	}, $.$model.xGet("moneyLend").xGet("ownerUser") !== Alloy.Models.User || $.$model.xGet("apportionType") === "Average"));
...
function deleteApportion() {
	if ($.$model.xGet("moneyLend").xGet("ownerUser") === Alloy.Models.User) {
		if ($.$model.isNew()) {
			...
			$.$model.xGet("moneyLend").xGet("moneyLendApportions").remove($.$model);
			updateAmount();
		} else {
			$.deleteModel();
		}
...
var oldAmount;
$.onWindowOpenDo(function() {
	// if ($.$model.xGet("apportionType") === "Fixed" && $.$model.xGet("moneyLend").xGet("ownerUser") === Alloy.Models.User) {
	// $.amount.$attrs.editModeEditability = "editable";
	// $.amount.$attrs.addModeEditability = "editable";
	// }
	oldAmount = $.$model.xGet("amount") || 0;
	$.$model.on("_xchange:amount", function() {
		if (($.$model.xGet("amount") || $.$model.xGet("amount") === 0) && $.$model.xGet("moneyLend").xGet("amount") && $.$model.apportionFocus) {
			$.$model.apportionFocus = false;
			if ($.$model.xGet("apportionType") === "Average") {
				$.$model.xSet("apportionType", "Fixed");
			}
			var fixedTotal = 0;
			$.$model.xGet("moneyLend").xGet("moneyLendApportions").forEach(function(item) {
				if (!item.__xDeletedHidden && !item.__xDeleted && item.xGet("apportionType") === "Fixed" && item !== $.$model) {
					fixedTotal = fixedTotal + item.xGet("amount");
				}
			});
			if ($.$model.xGet("amount") + fixedTotal > $.$model.xGet("moneyLend").xGet("amount")) {
				console.info("++amountValue++" + $.$model.xGet("amount") + "++++fixedTotal+++" + fixedTotal);
				alert("分摊总额大于实际支出金额(" + $.$model.xGet("moneyLend").xGet("amount") + ")，请重新调整");
...
function updateAmount() {
	...
	if (moneyLendApportionsArray.length > 0) {
		var lend = $.$model.xGet("moneyLend");
		var lendAmount = lend.xGet("amount") || 0;
		var averageApportions = [];
		var fixedTotal = 0;
		moneyLendApportionsArray.forEach(function(item) {
		...
		if ($.$model.hasChanged("apportionType") && $.$model.xGet("apportionType") === "Average" && (fixedTotal > $.$model.xGet("moneyLend").xGet("amount"))) {//若其他固定分摊总和大于收支总额，分摊从固定转成均摊不作操作
			alert("分摊总额大于实际支出金额(" + $.$model.xGet("moneyLend").xGet("amount") + ")，请重新调整");
		} else {
			if (averageApportions.length > 0) {
				...
				console.info("++averageTotal++" + averageTotal + "+++averageApportionsLength+++" + averageApportions.length + "++++++" + ($.$model.xGet("moneyLend").xGet("amount") - averageTotal));
				averageApportions[averageApportions.length - 1].xSet("amount", $.$model.xGet("moneyLend").xGet("amount") - averageTotal - fixedTotal);
