...
if ($.$attrs.modelType === "PersonalIncome") {
	...
	//querySelectApportion = "SELECT SUM(main.amount * mi.exchangeRate / IFNULL(ex.rate, 1)) AS END TOTAL FROM MoneyIncomeApportion main JOIN Project prj1 ON prj1.id = mi.projectId LEFT JOIN MoneyAccount ma ON ma.id = main.moneyAccountId LEFT JOIN Exchange ex ON ex.foreignCurrencyId = prj1.currencyId AND ex.localCurrencyId = '" + Alloy.Models.User.xGet("activeCurrencyId") + "' LEFT JOIN Exchange exma ON exma.localCurrencyId = ma.currencyId AND exma.foreignCurrencyId = '" + Alloy.Models.User.xGet("activeCurrencyId") + "'";
