...
if ($.$attrs.modelType === "PersonalIncome") {
	querySelect = "SELECT CASE WHEN main.ownerUserId = '" + Alloy.Models.User.id + "' THEN SUM(main.amount * IFNULL(exma.rate, 1)) ELSE SUM(main.amount * main.exchangeRate / IFNULL(ex.rate, 1)) END AS TOTAL FROM MoneyIncome main JOIN Project prj1 ON prj1.id = main.projectId LEFT JOIN MoneyAccount ma ON ma.id = main.moneyAccountId AND ma.currencyId = '" + Alloy.Models.User.xGet("activeCurrencyId") + "' LEFT JOIN Exchange ex ON ex.foreignCurrencyId = prj1.currencyId AND ex.localCurrencyId = '" + Alloy.Models.User.xGet("activeCurrencyId") + "' LEFT JOIN Exchange exma ON exma.foreignCurrencyId = ma.currencyId AND exma.localCurrencyId = '" + Alloy.Models.User.xGet("activeCurrencyId") + "'";
	//querySelectApportion = "SELECT SUM(main.amount * mi.exchangeRate / IFNULL(ex.rate, 1)) AS END TOTAL FROM MoneyIncomeApportion main JOIN Project prj1 ON prj1.id = mi.projectId LEFT JOIN MoneyAccount ma ON ma.id = main.moneyAccountId AND ma.currencyId = '" + Alloy.Models.User.xGet("activeCurrencyId") + "' LEFT JOIN Exchange ex ON ex.foreignCurrencyId = prj1.currencyId AND ex.localCurrencyId = '" + Alloy.Models.User.xGet("activeCurrencyId") + "' LEFT JOIN Exchange exma ON exma.foreignCurrencyId = ma.currencyId AND exma.localCurrencyId = '" + Alloy.Models.User.xGet("activeCurrencyId") + "'";
