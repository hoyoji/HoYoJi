...
var totalField = $.$attrs.totalField || "SUM(main.amount / IFNULL(ex.rate, 1))", value = 0, 
	query, queryApportion, queryLend, queryBorrow, queryReturnInterest, queryPaybackInterest,
	querySelect, querySelectApportion, querySelectLend, querySelectBorrow, querySelectReturnInterest, querySelectPaybackInterest;
if ($.$attrs.modelType === "PersonalIncome" || $.$attrs.modelType === "PersonalExpense") {
	if ($.$attrs.modelType === "PersonalIncome") {
		...
		querySelectApportion = "SELECT " + totalField + " AS TOTAL FROM MoneyIncomeApportion main JOIN Project prj1 ON prj1.id = mi.projectId LEFT JOIN Exchange ex ON ex.foreignCurrencyId = prj1.currencyId AND ex.localCurrencyId = '" + Alloy.Models.User.xGet("activeCurrencyId") + "' ";
